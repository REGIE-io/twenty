...
const SOURCE_FIELD_UNIVERSAL_IDENTIFIER =
  TIMELINE_ACTIVITY.fields.source.universalIdentifier;
...
@RegisteredWorkspaceCommand('2.32.0', 1789471410337)
@Command({
  name: 'upgrade:2-32:add-timeline-activity-source-field',
  description:
    'Add the timelineActivity.source system field on existing workspaces that predate it',
})
export class AddTimelineActivitySourceFieldCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    if (isDryRun) {
      this.logger.log(
        `[DRY RUN] Would add source field for workspace ${workspaceId}`,
      );
    ...
    if (result.status === 'fail') {
      this.logger.error(
        `Failed to add source field:\n${JSON.stringify(result, null, 2)}`,
      );

      throw new Error(
        `Failed to add source field for workspace ${workspaceId}`,
      );
    }

    this.logger.log(`Added source field for workspace ${workspaceId}`);
  }
}
