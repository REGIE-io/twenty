...
Field type is not currently an editable field-metadata property, so the current
API cannot perform an in-place `PHONES` to another-type transition. Deleting and
recreating a field is already covered. If type mutation is enabled later, the
same update handler should treat it as contribution create/delete; tests should
document that defensive behavior.
...
- Metadata identity and uniqueness tests proving one source field can target
  both vectors without changing existing generic contribution IDs.
- Expression tests for multiple standard/custom phone fields and additional
  phones, including assertions against PostgreSQL's `simple` parser.
...
- SQL/query-builder tests proving the dedicated path uses `@@` and never emits
  `LIKE`/`ILIKE`.
- `EXPLAIN` integration tests proving both a hit and a miss use the
  `phoneSearchVector` GIN index with sequential scans disabled; retain a sampled
  production-like plan check because tiny test tables can legitimately choose a
  sequential scan.
